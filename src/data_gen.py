...
def sea_mask_rnn(cfg, x, y, aux, climate, mask):
    # print('x shape are', x.shape)
    x = x.transpose(0,3,1,2)
    # print('x new shape are', x.shape)
    y = y.transpose(0,3,1,2)
    # print('aux shape are', aux.shape)
    aux = aux.transpose(2,0,1)
    # print('aux new shape are', aux.shape)
    # print('climate shape are', climate.shape)
    climate = climate.transpose(2, 0, 1)
    # print('climate new shape are', climate.shape)
   # scaler = scaler.transpose(0,3,1,2)
    ...
    climate = climate.reshape(climate.shape[0], climate.shape[1] * climate.shape[2])
    mask = mask.reshape(mask.shape[0]*mask.shape[1])
    x = x[:,:,mask==1]
    y = y[:,mask==1]
    aux = aux[:,mask==1]
    climate = climate[:, mask == 1]
    return x, y, aux, climate
