...
from meta_training import train_meta
...
def main(cfg):
    ...
    print('Now we training {d_p} product in {sr} spatial resolution'.format(d_p=cfg['product'],
                                                                            sr=str(cfg['spatial_resolution'])))
    print(
        '1 step:-----------------------------------------------------------------------------------------------------------------')
    print('[ATAI {d_p} work ] Make & load inputs'.format(d_p=cfg['workname']))
    path = cfg['inputs_path'] + cfg['product'] + '/' + str(
    ...
    if os.path.exists(path + '/x_train_norm.npy'):  
        ...
        x_train = np.memmap(path + 'x_train_norm.npy', dtype=cfg['data_type'], mode='r+',
                            shape=(x_train_shape[0], x_train_shape[1], x_train_shape[2], x_train_shape[3]))
        x_test_shape = np.load(path + 'x_test_norm_shape.npy', mmap_mode='r')
        x_test = np.memmap(path + 'x_test_norm.npy', dtype=cfg['data_type'], mode='r+',
        ...
        static_climate = np.load(path + 'static_climate_norm.npy')
        ...
        x_train, y_train, x_test, y_test, static, static_climate, lat, lon, mask = cls.fit(cfg)
        
    # load scaler for inverse
    if cfg['normalize_type'] in ['region']:
        scaler_x = np.memmap(path + 'scaler_x.npy', dtype=cfg['data_type'], mode='r+',
                             shape=(2, x_train.shape[1], x_train.shape[2], x_train.shape[3]))
        scaler_y = np.memmap(path + 'scaler_y.npy', dtype=cfg['data_type'], mode='r+',
                             shape=(2, y_train.shape[1], y_train.shape[2], y_train.shape[3]))
    ...
    if os.path.exists(out_path + cfg['modelname'] + '_para.pkl'):  
        ...
        for j in range(cfg["num_repeat"]):
            train_meta(x_train, y_train, static, static_climate, mask, scaler_x, scaler_y, cfg, j, path, out_path,
                       device)
            #train(x_train, y_train, static, static_climate, mask, scaler_x, scaler_y, cfg, j, path, out_path, device)
            model = torch.load(out_path + cfg['modelname'] + '_para.pkl')
    ...
    print(
        '[ATAI {d_p} work ] Saving predictions by {m_n} Model and we hope to use "postprocess" and "plot_test" codes for detailed analyzing'.format(
