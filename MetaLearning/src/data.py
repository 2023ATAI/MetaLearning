        ...
        if not os.path.exists(PATH + 'static_norm.npy'):
            ...
            for i in range(len(self.static_list)):
                ...
                with xr.open_dataset(file_static) as f_:
                    ...
                    if i != len(self.static_list)-1:
                        print(i)
                        print('max is', len(self.static_list))
                        static_data = self._interp(static_data,mask)
                print(i)
                print('End', i )
                static_input.append(static_data)
            static_input = np.stack(static_input, axis=-1)
            static_climate = static_input[:,:,-1]
            static = static_input[:,:,:-1]
            # np.savetxt("111.csv", static_climate, delimiter=",")
            static_climate = static_climate.reshape(static_climate.shape[0],static_climate.shape[1],1)
            np.save(PATH + 'static_climate_norm.npy', static_climate)
...
# save
        ...
        return x_train_norm, y_train_norm, x_test_norm, y_test_norm, static, static_climate, lat, lon, mask
#------------------------------------------------------------------------------------------------------------------------------
